...
export async function GET(
  ...
  try {
    ...
    console.error("[SKV int7] screenshot params error:", e);
  ...
  try {
    const res = await fetch(`${baseUrl}/api/screenshot/${encodeURIComponent(jobId)}`, {
      cache: "no-store",
    ...
    console.error("[SKV int7] screenshot proxy error:", error);
